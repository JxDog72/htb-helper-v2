...
def unique_capture_path(prefix: str, suffix: str = ".txt"):
    """logs/nmap_20260828_104015.txt — command plus date and time. A extra
    _2/_3 is added only if that exact second already has a file."""
    ws = STATE["workspace"]
    if not ws:
        raise RuntimeError("Workspace is not configured yet.")
    logs = engine.logs_dir(ws)
    ...
    dest = logs / f"{stem}_{stamp}{suffix}"
    number = 2
    while dest.exists():
